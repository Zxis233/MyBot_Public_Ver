...
divine = on_command("今日运势", aliases={"抽签", "运势"}, rule=to_me(), permission=GROUP, priority=8)
divine_specific = on_regex(r"^.+抽签$", rule=to_me(), permission=GROUP, priority=8)
limit_setting = on_regex(r"^指定(.*?)签$", rule=to_me(), permission=GROUP, priority=8)
theme_setting = on_regex(r"^设置(.*?)签$", rule=to_me(), permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=8,
                         block=True)
reset = on_regex("^重置(抽签)?主题$", rule=to_me(), permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=8,
                 block=True)
theme_list = on_fullmatch("主题列表", rule=to_me(), permission=GROUP, priority=8, block=True)
show = on_regex("^查看(抽签)?主题$", rule=to_me(), permission=GROUP, priority=8, block=True)
refresh = on_fullmatch("刷新抽签", rule=to_me(), permission=SUPERUSER, priority=7, block=True)
